...
All the SSGs I know of require doing it all manually,
maybe with a command to fill in the creation date.

```
---
title: Dates in your SSG
## DRAFT DATE - remember to change on publishing
...
It's pretty annoying and a bit error-prone to manage.

Instead, I'd like it to get the dates from the computer
and just save it to the file whenever it runs.
...
so the nicest way is using a push-hook.
Whenever I push to `origin/main`,
the hook will run the SSG to check
if I'm publishing any new posts,
and if I am, it'll add in a published date,
followed by a quick commit and push.
I'd also use a commit-hook to manage modification dates.
...
That's a big reason to use plain text and CLI tools.
